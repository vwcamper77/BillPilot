...
export default function SubscriptionActivation({ sessionId }) {
      ...
      .then(() => {
        ...
        unsubscribe = onAuthStateChanged(auth, async (user) => {
          ...
          try {
            ...
            if (!active) return;
            setState("active");
            setMessage("Your trial is active. You paid £0 today. Your first £1.99 payment is due after 7 days, then monthly unless you cancel.");
          } catch (error) {
            if (!active) return;
            setState("error");
  ...
  return (
    <>
      ...
      <h1>{state === "active" ? "Your ClearTill trial is active" : "Activating your ClearTill trial"}</h1>
      <p aria-live="polite">{message}</p>
      <div className="auth-button-row">
        <Link className="primary-button" href="/dashboard">
          {state === "active" ? "Open ClearTill" : "Continue to ClearTill"}
        </Link>
