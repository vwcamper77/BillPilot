...
const { validateSettings } = pipeline;
...
async function settings(db) {
  ...
  if (!snapshot.exists || !Number.isInteger(saved.batchSize)) {
...
async function claimNextJob(db, now) {
  ...
  return db.runTransaction(async (transaction) => {
    const batchRef = db.collection(COLLECTIONS.batches).doc(snapshot.docs[0].data().batchId);
    ...
    if (
      ...
      || ["cancelled", "partially_cancelled"].includes(batch.status)
    ) {
      transaction.update(jobRef, {
        status: "cancelled",
        cancelledAt: FieldValue.serverTimestamp(),
        cancellationReason: batch.cancellationReason || "Batch cancellation was requested.",
        updatedAt: FieldValue.serverTimestamp(),
      });
      ...
      transaction.set(db.collection(COLLECTIONS.calendar).doc(fresh.data().calendarItemId), {
        status: "planned",
...
async function failJob(db, job, error) {
  await db.runTransaction(async (transaction) => {
    ...
    const cancellationRequested = batch.cancellationRequested === true
      || ["cancelled", "partially_cancelled"].includes(batch.status);
    ...
    transaction.set(batchRef, {
      ...
      status: cancellationRequested
        ? "partially_cancelled"
        : terminal ? "completed_with_failures" : "in_progress",
      updatedAt: FieldValue.serverTimestamp(),
    }, { merge: true });
