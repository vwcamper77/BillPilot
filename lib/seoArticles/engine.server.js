...
export async function performSeoReviewAction({ token, note = "" }) {
  ...
  return {
    ...
    status,
    draftId: draft.draftId,
    version: Math.max(1, Number(draft.currentVersion || draft.version || 1) || 1),
...
export async function performAdminSeoReviewAction({
  draftId,
  versionId,
  action,
  note = "",
  ...
  if (!id || !["approve", "request_changes", "reject"].includes(selectedAction)) {
    const error = new Error("Choose a valid article and review action.");
    error.code = "seo/invalid-admin-review-action";
    throw error;
  }
  const db = getAdminDb();
  const [draftSnapshot, exportSnapshot] = await Promise.all([
  ...
  const currentVersion = `v${Math.max(
    1,
    Number(
      exportSnapshot.data()?.version
      || exportSnapshot.data()?.versionNumber
      || draft.currentVersion
      || draft.version
      || 1,
    ) || 1,
  )}`;
  if (String(versionId || "") !== currentVersion) {
    const error = new Error("This review targets a stale article version.");
    error.code = "seo/stale-version";
    throw error;
  }
  if (draft.status !== "in_review") {
    const error = new Error("This article review has already been completed.");
    error.code = "seo/review-already-completed";
    throw error;
  }
  return performSeoReviewAction({
    token: actionToken(draft, selectedAction),
    note,
  });
}
