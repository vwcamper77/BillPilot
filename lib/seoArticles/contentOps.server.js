...
async function readSettings(db) {
  ...
  return snapshot.exists ? validateSettings(snapshot.data()) : validateSettings(DEFAULT_SETTINGS);
}
...
async function updateCalendarBrief(db, actor, input) {
  ...
  const primaryKeyword = String(input.primaryKeyword || "").trim().toLowerCase();
  ...
  const result = await db.runTransaction(async (transaction) => {
    const [snapshot, keywordSnapshot] = await Promise.all([
      transaction.get(ref),
      transaction.get(
        db.collection(COLLECTIONS.calendar)
          .where("primaryKeyword", "==", primaryKeyword)
          .limit(2),
      ),
    ]);
    ...
    if (keywordSnapshot.docs.some((doc) => doc.id !== id)) {
    ...
    if (item.provisionalTitle === provisionalTitle && item.primaryKeyword === primaryKeyword) {
    ...
    transaction.update(ref, {
      ...
      rationale: String(input.rationale || item.rationale || "").trim(),
      updatedAt: FieldValue.serverTimestamp(),
    });
    writeTransition(transaction, db, {
      entityId: id,
...
async function createGenerationBatch(db, actor, input) {
  ...
  const requestedIds = new Set((input.calendarItemIds || []).map(String));
  const batchSize = Math.min(10, Math.max(1, Number(input.batchSize || settings.batchSize)));
  const selected = calendar
    .filter((item) => (
      requestedIds.size
        ? requestedIds.has(item.calendarItemId)
        : ["planned", "research_ready", "generation_failed"].includes(item.status)
    ))
    .slice(0, batchSize);
  if (!selected.length) throw new Error("No eligible calendar slots were selected.");
  const duplicate = selected.find((item) => !findDuplicateRisks(item, existingArticles).passed);
  if (duplicate) {
    const error = new Error(`Duplicate keyword or search intent detected for ${duplicate.calendarItemId}.`);
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    for (const [index, item] of selected.entries()) {
      const jobId = `${batchId}-${String(index + 1).padStart(2, "0")}`;
      transaction.create(db.collection(COLLECTIONS.jobs).doc(jobId), {
        jobId,
        batchId,
        calendarItemId: item.calendarItemId,
        brief: item,
