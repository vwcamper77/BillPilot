...
function planItem({
  ...
  if (kind === "adaptive") {
    return {
      calendarItemId,
      provisionalTitle: `Adaptive opportunity: ${titleCase(theme)}`,
      primaryKeyword: `adaptive topic ${publicationDate}`,
      secondaryKeywords: [],
      category: "adaptive",
      contentCluster: "adaptive search and content refresh",
      searchIntent: "To be selected from verified Search Console or current data",
      ...
      status: "planned",
      priority: "adaptive",
      rationale: "Reserved for Search Console evidence, a reactive topic, a data study or a content refresh. No traffic forecast is implied.",
      createdAt,
      updatedAt: createdAt,
    };
  }
  ...
  const articleType = brief.articleTypes[(categoryOrdinal - 1) % brief.articleTypes.length];
  return {
    calendarItemId,
    provisionalTitle: `${titleCase(angle)}: ${titleCase(theme)}`,
    primaryKeyword: `${angle} UK`,
    secondaryKeywords: [
      brief.keyword,
      `${brief.cluster} UK`,
      theme.split(",")[0],
    ...
    searchIntent: articleType === "product education"
    ...
    timezone,
    seasonalRelevance: theme,
    evergreenOrAdaptive: category === "seasonal_life_events" ? "seasonal" : "evergreen",
    articleType,
    proposedCta: brief.cta,
    proposedInternalLinks: brief.links,
    supportingAssetRequirement: articleType === "template"
    ...
    rationale: `Fulfils the annual ${brief.label.toLowerCase()} allocation and the ${month} theme. No keyword volume or traffic forecast has been invented.`,
    createdAt,
    updatedAt: createdAt,
  };
}
