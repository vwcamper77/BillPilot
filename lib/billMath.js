...
export function normaliseBillName(name) {
  ...
  const cleanedName = normaliseEntityName(
    originalName
      .replace(/\b(of each month|every month|each month|per month|each|monthly|month|due|on|the)\b/gi, "")
      .replace(/\s+/g, " ")
      .trim(),
  );

  console.log("[bill-name-cleanup]", {
    originalName,
    cleanedName,
  });
...
export function buildBillDocument(parsed, todayIso = getTodayIso()) {
  ...
  const schedule = calculateBillSchedule(safeDueDay, reminderOffsetDays, todayIso);
...
export function calculateDashboard(
  ...
  const runwayBills = paydayDate ? beforePayday : fallbackUpcomingBills;
  ...
  return {
    ...
    runwayTitle: paydayDate ? "Runway" : "Next bills",
    runwayEvents: buildRunwayEvents(runwayBills, paydayDate, todayIso),
  };
}
...
function buildRunwayEvents(bills, paydayDate, todayIso) {
  ...
  bills.forEach((bill) => {
    events.push({
      ...
      detail: bill.nextDueDate
        ? `${formatGBP(bill.amount)} due ${formatDueLabel(bill.nextDueDate, todayIso)}`
        : `${formatGBP(bill.amount)} due date not set`,
      type: "bill",
    });
  });
