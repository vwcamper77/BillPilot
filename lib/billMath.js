...
const UTILITY_SUPPLIER_MATCHERS = [
  { name: "British Gas", defaultCategory: "gas_electricity", patterns: [/\bbritish\s+gas\b/i] },
  { name: "Octopus Energy", defaultCategory: "gas_electricity", patterns: [/\boctopus(?:\s+energy)?\b/i] },
  { name: "EDF", defaultCategory: "gas_electricity", patterns: [/\bedf\b/i] },
  { name: "E.ON Next", defaultCategory: "gas_electricity", patterns: [/\be\.?\s*on(?:\s+next)?\b/i, /\beon(?:\s+next)?\b/i] },
  { name: "ScottishPower", defaultCategory: "gas_electricity", patterns: [/\bscottish\s*power\b/i] },
  { name: "Ovo Energy", defaultCategory: "gas_electricity", patterns: [/\bovo(?:\s+energy)?\b/i] },
  { name: "Shell Energy", defaultCategory: "gas_electricity", patterns: [/\bshell\s+energy\b/i] },
  { name: "Utility Warehouse", defaultCategory: "gas_electricity", patterns: [/\butility\s*warehouse\b/i] },
  { name: "Affinity Water", defaultCategory: "water", patterns: [/\baffinity\s+water\b/i] },
  { name: "Thames Water", defaultCategory: "water", patterns: [/\bthames\s+water\b/i] },
  { name: "Southern Water", defaultCategory: "water", patterns: [/\bsouthern\s+water\b/i] },
  { name: "South East Water", defaultCategory: "water", patterns: [/\bsouth\s+east\s+water\b/i] },
  { name: "Anglian Water", defaultCategory: "water", patterns: [/\banglian\s+water\b/i] },
  { name: "United Utilities", defaultCategory: "water", patterns: [/\bunited\s+utilities\b/i] },
  { name: "Yorkshire Water", defaultCategory: "water", patterns: [/\byorkshire\s+water\b/i] },
  { name: "Severn Trent Water", defaultCategory: "water", patterns: [/\bsevern\s+trent(?:\s+water)?\b/i] },
  { name: "Welsh Water", defaultCategory: "water", patterns: [/\bwelsh\s+water\b/i, /\bdwr\s*cymru\b/i] },
];
...
function detectUtilitySupplier(text) {
  if (!text) {
    return null;
  }

  return UTILITY_SUPPLIER_MATCHERS.find((supplier) => supplier.patterns.some((pattern) => pattern.test(text))) || null;
}
...
function stripMatchedSupplier(text, supplier) {
  ...
  return supplier.patterns.reduce(
    (current, pattern) => current.replace(pattern, " "),
    text,
  ).replace(/\s+/g, " ").trim();
}

function detectUtilityCategory(text) {
  if (!text) {
    return "";
  }
...
export function normaliseUtilityBillTitle(value) {
  ...
  const supplier = detectUtilitySupplier(cleaned);
  ...
  return supplier ? `${supplier.name} — ${categoryLabel}` : categoryLabel;
}

export function resolveBillTitle(name, sourceText = "") {
  const utilityFromSource = normaliseUtilityBillTitle(sourceText);

  if (utilityFromSource) {
    return utilityFromSource;
  }

  const utilityFromName = normaliseUtilityBillTitle(name);
...
export function normaliseBillName(name) {
  ...
  const originalName = String(name)
    .replace(/[’‘]/g, "'")
    .replace(/[–—]/g, "-");
  const utilityTitle = normaliseUtilityBillTitle(originalName);
