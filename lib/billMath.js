...
export function projectLargeCost(cost, todayIso = getTodayIso()) {
  ...
  if (frequency === "one_off") {
    ...
    const cappedPerDay = amount / 14;
    const perDayRaw =
      daysUntilDue <= 0
        ? amount
        : Math.min(amount / Math.max(1, daysUntilDue), cappedPerDay);
  ...
  const daysInPeriod = LARGE_COST_PERIOD_DAYS[frequency] || 365;
  const perDayRaw = amount / daysInPeriod;
...
export function largeCostDailyReserve(costs = [], todayIso = getTodayIso()) {
  return costs.reduce((total, cost) => {
    ...
    return total + (projected?.perDayRounded || 0);
  }, 0);
}
