...
import { calculateCashPosition, computeSafeDailyRate, expandIncomeEvents } from "@/lib/cashflowTimeline";
...
export function getFourWeekPayBuckets(todayIso, payDateIso, afterBillsAmount = 0, safeDailyAmount = 0, weekCount = 4) {
  ...
  const safeWeekCount = Math.max(1, Math.min(12, Math.floor(Number(weekCount) || 4)));
 ...
 * shared scale.
 *
 * Every bill, large cost and income occurrence in the visible window is
 * collected into a single chronological ledger, sorted by exact date (same-day
 * outgoings are applied before income), and applied to a running balance exactly
 * once. This guarantees a bill due after payday is never pulled into a later
 * week just because payday happens to fall in the same week, and that a week
 * containing payday correctly nets every transaction that occurs within it
 * regardless of whether it falls before or after the pay date.
 *
 * For the week containing payday, three extra checkpoints are exposed so the
 * chart can show the pre-payday and post-payday state separately instead of
 * collapsing the whole week into one number:
 * - prePaydayClosingBalance: balance after every transaction dated before payday
 * - paydayMomentBalance: balance immediately after the pay income lands
 * - postPaydayClosingBalance: balance after everything else later that week (== closingBalance)
 */
export function buildFourWeekCashflowWaterfall(todayIso, paydayDate, currentBalance, bills = [], largeCosts = [], incomeAmount = 0, additionalIncomeEvents = [], weekCount = 4) {
  const weekBuckets = getFourWeekPayBuckets(todayIso, paydayDate, currentBalance, 0, weekCount);
  const horizonEnd = weekBuckets[weekBuckets.length - 1]?.weekEnd || todayIso;
  const hasPayDate = isValidIsoDate(paydayDate);

  const canonicalPosition = calculateCashPosition({
    todayIso,
    horizonDate: horizonEnd,
    currentBalance,
    bills,
    largeCostAllocations: largeCosts,
    additionalIncomeEvents,
    paydayDate,
    incomeAmount,
  });
  const events = canonicalPosition.events.map(({ balanceAfter: _balanceAfter, ...event }) => event);
  // Same-day bills remain conservative (before income), while planned
  // large-cost contributions belong to the new pay period (after income).
  events.sort((a, b) => a.date.localeCompare(b.date) || a.rank - b.rank || a.type.localeCompare(b.type));

  let runningBalance = Number(currentBalance) || 0;
  let eventIndex = 0;
  const advanceThrough = (limitDateInclusive) => {
    const applied = [];
    while (eventIndex < events.length && events[eventIndex].date <= limitDateInclusive) {
      const event = events[eventIndex];
      runningBalance += event.amount;
      applied.push({ ...event, balanceAfter: runningBalance });
      eventIndex += 1;
    }
    return applied;
  };
  ...
  return weekBuckets.map((bucket) => {
    ...
    let steps;
    let prePaydayClosingBalance = null;
    let paydayMomentBalance = null;

    if (bucket.containsPayDate && hasPayDate) {
      const beforePayday = advanceThrough(addDaysIso(paydayDate, -1));
      prePaydayClosingBalance = runningBalance;
      const onPayday = advanceThrough(paydayDate);
      const paydayIncomeSteps = onPayday.filter((step) => step.amount > 0);
      paydayMomentBalance = paydayIncomeSteps.length
        ? paydayIncomeSteps[paydayIncomeSteps.length - 1].balanceAfter
        : prePaydayClosingBalance;
      const afterPayday = advanceThrough(bucket.weekEnd);
      steps = [...beforePayday, ...onPayday, ...afterPayday];
    } else {
      steps = advanceThrough(bucket.weekEnd);
    }

    const closingBalance = runningBalance;
    const weeklyOutflow = steps.filter((step) => step.amount < 0).reduce((total, step) => total - step.amount, 0);
    const incomeSteps = steps.filter((step) => step.amount > 0);
    const incomeReceived = incomeSteps.reduce((total, step) => total + step.amount, 0);
    const primaryIncome = incomeSteps.filter((step) => step.type === "primary_pay").reduce((total, step) => total + step.amount, 0);
    const additionalIncomeReceived = incomeReceived - primaryIncome;
    const incomeBreakdown = incomeSteps.map((step) => ({ type: step.type, name: step.name, date: step.date, amount: step.amount }));
    const primaryPayStep = incomeSteps.find((step) => step.type === "primary_pay");
    ...
    return {
      ...
      incomeBreakdown,
      closingBalance,
      containsPayDate: bucket.containsPayDate || Boolean(primaryPayStep),
      isAfterPayCycle: bucket.isAfterPayCycle,
      payDateLabel: primaryPayStep ? `Pay date ${formatShortDisplayDate(primaryPayStep.date)}` : bucket.payDateLabel,
      steps,
      prePaydayClosingBalance: bucket.containsPayDate ? prePaydayClosingBalance : null,
      paydayMomentBalance: bucket.containsPayDate ? paydayMomentBalance : null,
      postPaydayClosingBalance: bucket.containsPayDate ? closingBalance : null,
    };
  });
}
...
/**
 * Per-week "safe to spend" figures for the four-week chart. A projected bank
 * balance is not spending money — it includes money already earmarked for
 * bills, large costs and savings later in the cycle. This instead reports,
 * for every week in the window, the amount that is genuinely safe to spend
 * during that week: a daily safe-spend rate multiplied by the days of that
 * week which fall in the applicable pay period.
 *
 * Pre-payday and post-payday days get their own daily rate, each computed
 * independently by walking the same chronological ledger and finding the
 * worst-case balance-per-day ratio for that segment only (computeSafeDailyRate).
 * The post-payday rate assumes the same income recurs and is never seeded
 * from the pre-payday rate — a week after payday is a fresh budget, not last
 * week's £7/day carried forward. A week that contains payday shows both the
 * pre-payday and post-payday amounts split out, matching the split bars in
 * buildFourWeekCashflowWaterfall.
 */
export function buildWeeklySafeSpendingPlan(todayIso, paydayDate, currentBalance, bills = [], largeCosts = [], incomeAmount = 0, additionalIncomeEvents = [], weekCount = 4) {
  const waterfall = buildFourWeekCashflowWaterfall(todayIso, paydayDate, currentBalance, bills, largeCosts, incomeAmount, additionalIncomeEvents, weekCount);
  const hasPayDate = isValidIsoDate(paydayDate);
  const horizonEnd = waterfall[waterfall.length - 1]?.weekEnd || todayIso;

  let preDailyRate = 0;
  let postDailyRate = 0;
  let preShortfall = false;
  let postShortfall = false;
  ...
  if (hasPayDate) {
    ...
    for (const point of waterfall) {
      for (const step of point.steps) {
        // The dashboard's canonical income schedule can supply payday through
        // additionalIncomeEvents rather than incomeAmount. Regardless of its
        // source type, income landing on the boundary is not available before
        // that income. The waterfall has already included it in the payday
        // moment balance used to open the post-income period.
        if (step.date === paydayDate && step.amount > 0) continue;
        if (step.date === paydayDate && step.type === "large_cost") {
          postEvents.push(step);
          continue;
        }
        // Same-day bills must still be covered before relying on payday income.
        (step.date <= paydayDate ? preEvents : postEvents).push(step);
      }
    }

    const pre = computeSafeDailyRate({ openingBalance: currentBalance, fromIso: todayIso, throughIso: paydayDate, events: preEvents });
    preDailyRate = pre.rawSafeDailyAmount ?? pre.safeDailyAmount;
    preShortfall = pre.minimumProjectedBalance < 0;

    const paydayPoint = waterfall.find((point) => point.containsPayDate);
    const paydayMomentBalance = paydayPoint ? paydayPoint.paydayMomentBalance : null;
    if (paydayMomentBalance !== null && paydayDate < horizonEnd) {
      const post = computeSafeDailyRate({
        openingBalance: paydayMomentBalance,
        fromIso: paydayDate,
        throughIso: addDaysIso(horizonEnd, 1),
        events: postEvents,
      });
      postDailyRate = post.rawSafeDailyAmount ?? post.safeDailyAmount;
      postShortfall = post.minimumProjectedBalance < 0;
    }
  } else {
    const rollingEvents = waterfall.flatMap((point) => point.steps);
    const rolling = computeSafeDailyRate({
      openingBalance: currentBalance,
      fromIso: todayIso,
      throughIso: addDaysIso(horizonEnd, 1),
      events: rollingEvents,
    });
    preDailyRate = rolling.rawSafeDailyAmount ?? rolling.safeDailyAmount;
    preShortfall = rolling.minimumProjectedBalance < 0;
  }

  return waterfall.map((point) => {
    const weekStart = point.weekStart;
    const weekEnd = point.weekEnd;
    const effectiveStart = weekStart < todayIso ? todayIso : weekStart;

    let preDays = 0;
    let postDays = 0;
    if (!hasPayDate) {
      preDays = diffDays(effectiveStart, weekEnd) + 1;
    } else if (paydayDate > weekEnd) {
      preDays = diffDays(effectiveStart, weekEnd) + 1;
    } else if (paydayDate <= effectiveStart) {
      postDays = diffDays(effectiveStart, weekEnd) + 1;
    } else {
      preDays = diffDays(effectiveStart, addDaysIso(paydayDate, -1)) + 1;
      postDays = diffDays(paydayDate, weekEnd) + 1;
    }

    const preAvailableToSpend = preDays > 0 ? roundMoney(preDailyRate * preDays) : 0;
    const postAvailableToSpend = postDays > 0 ? roundMoney(postDailyRate * postDays) : 0;
    const isSplit = preDays > 0 && postDays > 0;
    const weeklyMinimumBalance = Math.min(point.openingBalance, ...point.steps.map((step) => step.balanceAfter));
    const isShortfall = weeklyMinimumBalance < 0;
    const displayedPreAvailable = isSplit && isShortfall ? roundMoney(weeklyMinimumBalance) : preAvailableToSpend;

    return {
      ...point,
      // Keep bills and planned large-cost allocations mutually exclusive in
      // the UI. Both are already applied once in the chronological ledger.
      billsDue: roundMoney(point.steps.filter((step) => step.type === "bill").reduce((total, step) => total + Math.abs(step.amount), 0)),
      largeCostAllocations: roundMoney(point.steps.filter((step) => step.type === "large_cost").reduce((total, step) => total + Math.abs(step.amount), 0)),
      projectedClosingBalance: point.closingBalance,
      preDays,
      postDays,
      preDailyRate: preDays > 0 ? preDailyRate : null,
      postDailyRate: postDays > 0 ? postDailyRate : null,
      preAvailableToSpend: preDays > 0 ? displayedPreAvailable : null,
      postAvailableToSpend: postDays > 0 ? postAvailableToSpend : null,
      availableToSpend: isShortfall ? roundMoney(weeklyMinimumBalance) : roundMoney(preAvailableToSpend + postAvailableToSpend),
      dailyRate: isSplit ? null : preDays > 0 ? preDailyRate : postDays > 0 ? postDailyRate : null,
      isShortfall,
      weeklyMinimumBalance: roundMoney(weeklyMinimumBalance),
    };
  });
}
