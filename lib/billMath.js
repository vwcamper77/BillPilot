...
export function buildFourWeekCashflowWaterfall(todayIso, paydayDate, currentBalance, bills = [], largeCosts = [], incomeAmount = 0, additionalIncomeEvents = []) {
  ...
  if (hasPayDate && safeIncomeAmount > 0 && paydayDate <= horizonEnd) {
    events.push({ date: paydayDate, amount: safeIncomeAmount, type: "primary_pay", rank: 0, name: "Pay" });
  }
 ...
 */
export function buildWeeklySafeSpendingPlan(todayIso, paydayDate, currentBalance, bills = [], largeCosts = [], incomeAmount = 0, additionalIncomeEvents = []) {
  const waterfall = buildFourWeekCashflowWaterfall(todayIso, paydayDate, currentBalance, bills, largeCosts, incomeAmount, additionalIncomeEvents);
  const hasPayDate = isValidIsoDate(paydayDate);
  ...
  if (hasPayDate) {
    ...
    for (const point of waterfall) {
      for (const step of point.steps) {
        if (step.type === "primary_pay") continue; // baked into the post-payday opening balance below
