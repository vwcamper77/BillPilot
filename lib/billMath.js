...
export function buildFourWeekCashflowWaterfall(todayIso, paydayDate, currentBalance, bills = [], largeCosts = [], incomeAmount = 0, additionalIncomeEvents = [], weekCount = 4) {
  ...
  // Conservative same-day ordering: outgoings are applied before income so a
  // bill is never treated as safely covered by money that has not cleared yet.
  ...
  if (hasPayDate) {
    ...
    for (const point of waterfall) {
      for (const step of point.steps) {
        if (step.type === "primary_pay" && step.date === paydayDate) continue;
        // Same-day outgoings must be covered before relying on payday income.
        // The post-payday opening below already includes every payday event.
