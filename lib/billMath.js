 ...
 * balance, outflow, and closing balance, so the chart can render one bar per
 * week from openingBalance to closingBalance on a single shared scale.
 */
export function buildFourWeekCashflowWaterfall(todayIso, paydayDate, currentBalance, bills = [], largeCosts = []) {
  ...
  for (const bill of bills) {
    if (bill?.nextDueDate && Number(bill.amount) > 0) {
      weeklyOutflows[bucketForDate(bill.nextDueDate)] += Number(bill.amount);
    }
  }
  for (const cost of largeCosts) {
    const acctAmount = Number(cost?.currentAccountAmount) || 0;
    if (acctAmount > 0 && cost?.nextDueDate) {
      weeklyOutflows[bucketForDate(cost.nextDueDate)] += acctAmount;
    }
  }
  ...
  return weekBuckets.map((bucket, index) => {
    ...
    const closingBalance = openingBalance - weeklyOutflow;
    const point = {
