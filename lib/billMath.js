...
import { computeSafeDailyRate, expandIncomeEvents } from "@/lib/cashflowTimeline";
 ...
 * collected into a single chronological ledger, sorted by exact date (income
 * clears before same-day outgoings), and applied to a running balance exactly
...
export function buildFourWeekCashflowWaterfall(todayIso, paydayDate, currentBalance, bills = [], largeCosts = [], incomeAmount = 0, additionalIncomeEvents = [], weekCount = 4) {
  ...
  for (const bill of bills || []) {
    const firstDate = bill?.nextDueDate;
    const amount = Number(bill?.amount) || 0;
    if (firstDate && amount > 0) {
      let date = firstDate;
      let occurrence = 0;
      while (date <= horizonEnd) {
        if (date >= todayIso) {
          events.push({ date, amount: -amount, type: "bill", rank: 1, name: bill.name || "Bill" });
        }
        if (bill.frequency === "one_off") break;
        occurrence += 1;
        if (occurrence > 12) break;
        date = addMonthsIso(firstDate, occurrence);
      }
    }
  }
  for (const cost of largeCosts || []) {
    const firstDate = cost?.nextDueDate || cost?.date;
    const amount = Number(cost?.currentAccountAmount ?? cost?.amount) || 0;
    if (firstDate && amount > 0) {
      const monthStep = LARGE_COST_MONTH_STEPS[cost.frequency];
      let date = firstDate;
      let occurrence = 0;
      while (date <= horizonEnd) {
        if (date >= todayIso) {
          events.push({ date, amount: -amount, type: "large_cost", rank: 1, name: cost.name || "Large cost" });
        }
        if (!monthStep) break;
        occurrence += 1;
        if (occurrence > 12) break;
        date = addMonthsIso(firstDate, occurrence * monthStep);
      }
    }
  }
  const safeIncomeAmount = Number(incomeAmount) > 0 ? Number(incomeAmount) : 0;
  if (hasPayDate && safeIncomeAmount > 0) {
    let date = paydayDate;
    let occurrence = 0;
    while (date <= horizonEnd) {
      if (date >= todayIso) {
        events.push({ date, amount: safeIncomeAmount, type: "primary_pay", rank: 0, name: "Pay" });
      }
      occurrence += 1;
      if (occurrence > 12) break;
      date = addMonthsIso(paydayDate, occurrence);
    }
  }
  const incomeOccurrences = expandIncomeEvents(additionalIncomeEvents, addDaysIso(todayIso, 1), horizonEnd);
  for (const occurrence of incomeOccurrences) {
    events.push({ date: occurrence.date, amount: occurrence.amount, type: "additional_income", rank: 0, name: occurrence.name });
  }

  // Sort by exact date; on a shared date, income clears before outgoings so a
  // payday bill is never treated as paid from money that hasn't landed yet.
  ...
  return weekBuckets.map((bucket) => {
    ...
    if (bucket.containsPayDate && hasPayDate) {
      ...
      const primaryPayStep = onPayday.find((step) => step.type === "primary_pay");
      paydayMomentBalance = primaryPayStep ? primaryPayStep.balanceAfter : prePaydayClosingBalance;
...
export function buildWeeklySafeSpendingPlan(todayIso, paydayDate, currentBalance, bills = [], largeCosts = [], incomeAmount = 0, additionalIncomeEvents = [], weekCount = 4) {
  ...
  if (hasPayDate) {
    ...
    for (const point of waterfall) {
      for (const step of point.steps) {
        if (step.type === "primary_pay" && step.date === paydayDate) continue; // baked into the post-payday opening balance below
  ...
  return waterfall.map((point) => {
    ...
    return {
      ...
      preAvailableToSpend: preDays > 0 ? preAvailableToSpend : null,
      postAvailableToSpend: postDays > 0 ? postAvailableToSpend : null,
      availableToSpend: roundMoney(preAvailableToSpend + postAvailableToSpend),
      dailyRate: isSplit ? null : preDays > 0 ? preDailyRate : postDays > 0 ? postDailyRate : null,
      isShortfall: (preDays > 0 && preShortfall) || (postDays > 0 && postShortfall),
    };
  });
}
