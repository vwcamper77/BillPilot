...
export async function grantFoundingAccessByEmail(email) {
  ...
  const record = await grantFoundingAccessToUser({
    ...
    accessStartDate: new Date(),
    accessSource: "internal_promo",
    amountPaid: 0,
  });

  const now = new Date();
  const expiresAt = addMonths(now, FOUNDING_ACCESS_MONTHS);
  await getAdminDb().collection("pendingEntitlements").doc(`internal_${userRecord.uid}`).set({
    uid: userRecord.uid,
    status: "active",
    ...
    promotionLabel: "Internal promotional access",
    updatedAt: FieldValue.serverTimestamp(),
    createdAt: FieldValue.serverTimestamp(),
  }, { merge: true });
