...
export function classifyIncomeSource(source = {}) {
  if (source.active === false || String(source.status || "").toLowerCase() === "paused") return "paused";
  if (["cancelled", "canceled", "excluded"].includes(String(source.status || "").toLowerCase())) return "excluded";
  if (String(source.confidence || "confirmed").toLowerCase() === "estimated") return "estimated";
  return "confirmed";
}
...
export function normaliseIncomeSource(source = {}, fallbackId = null) {
  ...
  return {
    ...
    frequency,
    endDate,
    active: classification !== "paused" && classification !== "excluded",
    confidence: classification === "estimated" ? "estimated" : confidence,
    classification,
    occurrenceStatuses,
  };
}
...
export function hasActiveIncomeSchedule(incomeSources = []) {
  return (incomeSources || []).some((source) => {
    const normalised = normaliseIncomeSource(source);
    return ["confirmed", "estimated"].includes(normalised.classification)
      && Number(normalised.amount) > 0
      && Boolean(normalised.firstPaymentDate);
  });
}

export function upsertIncomeSource(incomeSources = [], savedSource = {}) {
  const saved = normaliseIncomeSource(savedSource, savedSource.id || null);
  return (incomeSources || []).some((source) => source.id === saved.id)
    ? incomeSources.map((source) => source.id === saved.id ? saved : source)
    : [...incomeSources, saved];
}
