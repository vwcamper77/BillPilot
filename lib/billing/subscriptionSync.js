...
export function mapStripeSubscription(subscription, extras = {}) {
  ...
  return {
    ...
    planName: subscription.items?.data?.[0]?.price?.nickname || subscription.metadata?.planName || extras.planName || "ClearTill",
    planAmountMinor: Number(subscription.items?.data?.[0]?.price?.unit_amount || extras.planAmountMinor) || null,
    planCurrency: subscription.items?.data?.[0]?.price?.currency || extras.planCurrency || null,
    billingInterval: subscription.items?.data?.[0]?.price?.recurring?.interval || extras.billingInterval || null,
    ...
    lastCheckoutSessionId: extras.lastCheckoutSessionId || null,
    internalTest: Boolean(extras.internalTest),
    excludedFromCommercialReporting: Boolean(extras.excludedFromCommercialReporting),
  };
}
