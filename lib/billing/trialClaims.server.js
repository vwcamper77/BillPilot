...
export async function createTrialCheckoutIntent({ anonymousUid, normalizedEmail }) {
...
async function sendTrialClaimEmail(checkoutIntentId, rawToken, email, { allowSent = false } = {}) {
  ...
  try {
    ...
    const result = await sendEmail({ to: email, ...buildWelcomeEmail(signInUrl) });
...
export async function claimTrialSubscription({ checkoutIntentId, claimToken, authenticatedUid, authenticatedEmail }) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const refs = [sourceUser, targetUser, sourceSubscription, sourceEntitlement, targetSubscription, targetEntitlement];
    ...
    const [sourceUserSnapshot, targetUserSnapshot, sourceSubscriptionSnapshot, sourceEntitlementSnapshot, targetSubscriptionSnapshot] = snapshots;
