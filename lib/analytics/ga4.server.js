...
/**
 * A hung (not rejected) fetch — e.g. a network path that never resolves —
 * would otherwise block every caller indefinitely, including ones that only
 * wrapped this in .catch(), which never fires for a promise that's merely
 * pending forever. Every GA4 network call goes through this.
 */
...
/**
 * One-way identifier for GA4 transaction_id/dedup fields that must not carry
 * a raw Stripe session ID or other reversible identifier into analytics.
 */
export function hashForAnalytics(value) {
  if (!value) {
    return null;
  }
...
export async function sendGa4Event({ eventName, clientId, userId = null, params = {}, forceDebug = false }) {
  ...
  const debug = forceDebug || process.env.GA4_DEBUG === "true";
  ...
  if (debug) {
    ...
    const validationResponse = await fetchWithTimeout(validationUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
