...
import { normalizePreviewRecord } from "@/lib/previewLifecycle.server";
import previewPolicy from "@/lib/previewPolicy.cjs";

const { overlayPreviewAccess } = previewPolicy;
...
export function normalizeEntitlementState({
  ...
  const preview = normalizePreviewRecord(previewRecord, now);
  const hasPriorCommercialAccess = subscriptionStatus !== "none"
    || Boolean(trialStartedAt || stripeSubscriptionId || canonical)
    || Boolean(Object.keys(legacyBilling || {}).length && (legacyBilling.status || legacyBilling.paid || legacyBilling.accessStatus));
  const { paidOrGrantedAccess, canStartPreview, hasAccess, canEdit, accessType, reason } = overlayPreviewAccess({
    decision,
    preview,
    isAuthenticated,
    hasPriorCommercialAccess,
  });
  const paymentPending = decision.paymentPending;
  ...
  const paidAccessExpiresAt = dateIso(canonical?.accessExpiresAt || canonical?.accessUntil || legacyBilling.accessExpiresAt || legacyBilling.accessUntil || user.accessUntil);
  const accessExpiresAt = paidOrGrantedAccess ? paidAccessExpiresAt : preview.endsAt;
  ...
  return {
    ...
    previewEndsAt: preview.endsAt,
    previewDaysRemaining: preview.daysRemaining,
    previewUsed: preview.used,
    canStartPreview,
    subscriptionStatus,
    trialStatus,
    trialStartedAt,
...
export async function resolveEntitlementForUid(uid, { accountEmail = null } = {}) {
  ...
  const [userSnap, subscriptionSnap, subscriptionEntitlementSnap, previewSnap, canonicalSnap, legacySnap, customerSnap] = await Promise.all([
    userRef.get(),
  ...
  return normalizeEntitlementState({
    ...
    previewRecord: previewSnap.exists ? previewSnap.data() : null,
...
export async function hasActiveEntitlement(uid, options = {}) {
  ...
  return {
    ...
    state: entitlement.hasAccess ? "access_active"
      : entitlement.canStartPreview ? "preview_available"
      : entitlement.paymentPending ? "payment_pending"
        : entitlement.previewUsed ? "access_expired"
