...
export function getAccessExpiryDate(accessRecord) {
  return toDateMaybe(accessRecord?.accessUntil)
    || toDateMaybe(accessRecord?.accessExpiresAt);
}

export function hasActiveBillingAccess(billingRecord, now = new Date()) {
  const expiresAt = getAccessExpiryDate(billingRecord);
