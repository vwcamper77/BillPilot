import { normaliseIncomeSource } from "@/lib/incomeSchedule";
...
export function isConfirmedIncomeEvent(event) {
  const source = normaliseIncomeSource(event);
  return source.classification === "confirmed"
...
export function expandIncomeEvents(events = [], fromIso, throughIso, { confirmedOnly = true, includeNonForecast = false, asOfIso = fromIso } = {}) {
  ...
  for (const rawEvent of events || []) {
    const event = normaliseIncomeSource(rawEvent);
    if (event.classification === "paused" || event.classification === "excluded") continue;
    if (confirmedOnly && event.classification !== "confirmed") continue;
    ...
    while (occurrenceDate <= throughIso && (!endDate || occurrenceDate <= endDate)) {
      if (occurrenceDate >= fromIso) {
        ...
        if (includeNonForecast || status === "scheduled") occurrences.push({
          ...
          date: occurrenceDate,
          frequency,
          confidence: event.classification === "estimated" ? "estimated" : "confirmed",
...
export function resolveNextConfirmedIncome(events = [], fromIso, throughIso = null) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(fromIso || ""))) return null;
  const searchThrough = throughIso || addMonths(fromIso, 60);
  return expandIncomeEvents(events, fromIso, searchThrough, { confirmedOnly: true })[0] || null;
}
    ...
    while (date <= horizonDate) {
      if (date >= todayIso) {
        occurrences.push({
          ...
          amount: -amount,
          type,
          // Bills must be covered before same-day income clears. Planned
          // large-cost contributions are allocations from that pay period,
          // so they land after income instead of reducing the prior period.
          rank: type === "bill" ? 0 : 2,
        });
      }
      if (!monthStep) break;
 ...
 * Same-day bills are deliberately applied before income; planned large-cost
 * contributions are applied after it (bill 0, income 1, contribution 2).
 */
export function calculateCashPosition({
  ...
  const outflowsBeforeNextIncome = nextConfirmedIncome
    ? outflowEvents.filter((event) => (
      event.date < preIncomeBoundary
      || (event.date === preIncomeBoundary && event.type === "bill")
    ))
    : outflowEvents.filter((event) => event.date <= preIncomeBoundary);
  ...
  const incomeRankByDate = new Map();
  for (const event of incomeEvents) {
    incomeRankByDate.set(event.date, Math.min(incomeRankByDate.get(event.date) ?? Number.POSITIVE_INFINITY, event.rank));
  }
  const sameDayDependencies = [...new Set(outflowEvents
    .filter((event) => incomeRankByDate.has(event.date) && event.rank < incomeRankByDate.get(event.date))
