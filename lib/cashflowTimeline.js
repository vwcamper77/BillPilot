...
export function calculateSafeSpendingPlan({
  ...
  // The period summary is a straight reconciliation of all included cash:
  // opening balance + confirmed income - commitments. Keep it separate from
  // `spendingRoom`, which is deliberately timing-sensitive, so consumers do
  // not accidentally mix the two definitions in one piece of UI.
  const availableBeforeHorizon = money(projectedClosingBalance);
  const dailyAvailableAmount = availableBeforeHorizon >= 0
    ? money(availableBeforeHorizon / days)
    : 0;
  return {
    days,
    safeDailyAmount,
    spendingRoom,
    availableBeforeHorizon,
