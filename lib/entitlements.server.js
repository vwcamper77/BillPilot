...
/**
 * Determines whether a Checkout Session is the exact paid founding offer.
 * Metadata is supplementary: fulfilment also requires the server-configured
 * Stripe Price, one line item, payment mode, a completed/paid session, and
 * the expected £5 GBP total.
 */
export async function getCheckoutFulfillability(session) {
  ...
  const expectedPriceId = String(process.env.STRIPE_PRICE_ID || "").trim();
  const lineItems = Array.isArray(session.line_items?.data) ? session.line_items.data : [];
  const lineItem = lineItems[0] || null;
  const actualPriceId = String(lineItem?.price?.id || "").trim();
  const expectedFoundingPrice = Boolean(
    expectedPriceId
      && lineItems.length === 1
      && actualPriceId === expectedPriceId
      && Number(lineItem?.quantity) === 1,
  );
  const fulfillable = paidPolicy.paid && expectedFoundingPrice;
