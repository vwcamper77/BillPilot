...
export async function getCheckoutFulfillability(session) {
  ...
  const fulfillable = session.status === "complete"
    && (isPaid || (isNoPaymentRequired && isZeroTotal && isConfirmedClear100));
 ...
 */
export async function createPendingEntitlementFromCheckoutSession(session) {
  ...
  const outcome = await db.runTransaction(async (transaction) => {
    ...
    if (evtSnapshot.exists || entSnapshot.exists) {
      const existing = entSnapshot.exists ? entSnapshot.data() : null;
      return { isNewSession: false, maskedEmail: existing?.maskedEmail || maskedEmail };
    }
    ...
    transaction.set(pendingCustomerRef(sessionId), {
      ...
      stripeCustomerId: session.customer ? String(session.customer) : null,
      currency,
      paymentStatus: isQaPurchase ? "qa" : "paid",
      totalPaid: isQaPurchase ? 0 : amountPaid,
    ...
    transaction.set(evtRef, {
      ...
      amountPaid,
      currency,
      ga4EventName: isQaPurchase ? "qa_purchase" : "purchase",
      createdAt: FieldValue.serverTimestamp(),
    });

    return { isNewSession: true, maskedEmail };
  }).catch((error) => {
  ...
  if (outcome.isNewSession) {
    await sendGa4Event({
      ...
      params: {
        transaction_id: hashForAnalytics(sessionId),
    ...
    }).catch((error) => console.error("[entitlements] ga4 purchase send failed", { sessionId }, error));
...
async function attemptSend(sessionId, claimToken, email, accessExpiresAt) {
  const signInUrl = await buildSignInUrl(sessionId, claimToken, email);

  try {
    await sendAccessLinkEmail({ to: email, signInUrl, accessExpiresAt });
    await recordEmailOutcome(sessionId, { ok: true });
  } catch (error) {
    console.error("[entitlements] access-link email send failed", { sessionId }, error);
    await recordEmailOutcome(sessionId, { ok: false, error: error?.message || "send failed" });
  }
}
...
async function recordEmailOutcome(sessionId, { ok, error }) {
 ...
 */
export async function resendAccessEmail({ sessionId }) {
  const { claimToken, email, data } = await rotateClaimToken(sessionId);
  await attemptSend(sessionId, claimToken, email, data.accessExpiresAt);
...
export async function claimPendingEntitlement({ sessionId, uid, verifiedEmail }) {
  ...
  const result = await getAdminDb().runTransaction(async (transaction) => {
    ...
    transaction.set(customerRef, {
      ...
      currency: entitlement.currency,
      paymentStatus: entitlement.isQaPurchase ? "qa" : "paid",
