...
export function formatAccessExpiry(date) {
  const resolved = date?.toDate ? date.toDate() : date instanceof Date ? date : new Date(date);
  if (Number.isNaN(resolved.getTime())) throw new Error("Invalid access expiry date.");
