...
function resolveSender(senderType) {
  if (senderType === "seo_review" && String(process.env.SEO_REVIEW_EMAIL_FROM || "").trim()) {
    return String(process.env.SEO_REVIEW_EMAIL_FROM).trim();
  }
  return ALLOWED_SENDERS[senderType] || CANONICAL_EMAIL_SENDER;
}
